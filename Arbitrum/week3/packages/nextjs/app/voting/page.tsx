...
const VotePage = () => {
  ...
  const {
    data: voterStatus,
    isLoading: isLoadingVoterStatus,
  ...
  const {
    writeContractAsync: resetVotes,
    isMining: isResetting,
    error: resetError,
  } = useScaffoldWriteContract("Voting");
      ...
      {isDisconnected ? (
        ...
        <div>
          <p className="text-lg mb-6 text-center">
            Connected account: <span className="font-semibold">{address}</span>
          </p>
                    ...
                    {!hasVoted && (
                      <button
                        className={`ml-4 bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded ${
                          selectedCandidate === index ? "opacity-50 cursor-not-allowed" : ""
                        }`}
